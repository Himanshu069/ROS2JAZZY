...
# Import ROS2 Node as parent for our own node class
...
class MyPublisherNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("published_text", "MOGI")
        self.declare_parameter("timer_period", 1.0)
        # Add the timer_period parameter with default 1s
    ...
    def timer_callback(self):
        # use the text_ variable for the String message
...
def main(args=None):
    ...
    rclpy.spin(node)
    # Keeps the node running until it's closed with ctrl+c
    node.destroy_node()
    rclpy.shutdown()
